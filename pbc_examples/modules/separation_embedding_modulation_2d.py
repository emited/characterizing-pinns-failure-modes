...
class SeparationEmbeddingMod2d(torch.nn.Module, EmbeddingModule):
    """Add central embedding to the main network"""
    def __init__(self, num_samples):
        super(SeparationEmbeddingMod2d, self).__init__()
        self.latent_dim = 128
        num_blocks = 4
        num_xt_blocks = 4
        hidden_dim = 128
        ...
        with torch.no_grad():
            ...
            self.ey.weight.data = torch.randn(self.ey.weight.shape) / math.sqrt(100)
        # with torch.no_grad():
        #     xbound = 1 / emb_dim
