...
class SeparationEmbeddingMod2dIntegrate(torch.nn.Module, EmbeddingModule):
    """Add central embedding to the main network"""
    def __init__(self, num_samples):
        super(SeparationEmbeddingMod2dIntegrate, self).__init__()
        # self.latent_dim = 128
        # num_blocks = 4
        # num_xt_blocks = 4
        # hidden_dim = 128
        ...
        self.ey = torch.nn.Embedding(num_samples, emb_dim)
        ...
        self.zt_func = EulerNet(self.latent_dim, hidden_dim, use_aux=True, aux_dim=emb_dim, steps=1)
        ...
        self.emb_param_group = list(self.ey.parameters())
        self.emb_param_group.extend(list(self.et.parameters()))
        self.emb_param_group.extend(list(self.et0.parameters()))
        # self.x_param_group = []
    ...
    def forward(self, x, y, t, sample_index=None, ex=None, ey=None, et=None):
        ...
        zt = self.zt_func(t, aux=et, x0=et0)
        # zt = zt * zt
        ...
        for b in self.blocks:
            ztpx = torch.cat([zt_broadcasted, px_broadcasted, py_broadcasted], -1)
            h = b(h, ztpx)
        u_pred = self.d(h)
