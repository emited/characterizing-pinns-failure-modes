...
if __name__ == '__main__':
    #  num_traj_per_env, size, time_horizon, dt_eval, params, n_block, dx=2.,
    params = [
    ...
    d = dataset_test[2]['state'] # torch.Size([2, 10, 32, 32])
    import matplotlib.pyplot as plt
    plt.figure(figsize=(12, 3))
    for i, x in enumerate(d[0]):
        plt.subplot(2, len(d[0]), i+1)
        plt.imshow(x)
        plt.subplot(2, len(d[0]), len(d[0])+i+1)
        plt.imshow(d[1][i])
    plt.show()